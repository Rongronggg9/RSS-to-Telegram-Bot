...
from .medium import construct_images_weserv_nl_url
...
class TelegraphIfy:
    def __init__(self, xml: str = None, title: str = None, link: str = None, feed_title: str = None,
        ...
        for tag in soup.find_all(recursive=True):
            ...
            for attr in tuple(tag.attrs.keys()):
                if attr not in ALLOWED_ATTRS:
                    ...
                    continue
                if attr == 'src' and not (
                        tag.attrs[attr].startswith(env.IMAGES_WESERV_NL)
                        or tag.attrs[attr].startswith(env.IMG_RELAY_SERVER)):
                    if tag.name == 'video':
                        tag.attrs[attr] = env.IMG_RELAY_SERVER + tag.attrs[attr]
                    elif tag.name == 'img':
                        tag.attrs[attr] = construct_images_weserv_nl_url(
                            url=tag.attrs[attr],
                            width=None, height=None, fit=None, output_format=None, without_enlargement=None,
                            default_image='https://via.placeholder.com/960x540?text=broken image'
                        )
        ...
        self.telegraph_html_content = (str(soup) +
                                       ...
                                       + (f"<br><a href='{link}'>Source</a>" if link else ''))
